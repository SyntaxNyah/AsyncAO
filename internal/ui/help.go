...
var glossaryEntries = []glossaryEntry{
	...
	{"HDID — Hardware ID", "A per-device identifier the client sends so a server can ban a device, not just an account. AsyncAO sends a DIFFERENT one to every server, so no operator holds an ID that works anywhere else. The Privacy section spells out exactly what it sends."},
...
var privacySections = []privacySection{
	...
	{"Your device ID (HDID)", []string{
		...
		"That hash is then TIED TO THE SERVER you're joining: what a server actually receives is a hash of its own address plus your device hash, so the same PC shows a completely different ID on every server. This is the part that matters if you worry about being impersonated. With one ID everywhere — which is what every other AO client sends — anyone who reads it (an operator, anyone with access to a ban list, anyone sniffing an unencrypted ws:// connection) holds a value that IS your device on every server there is: they can send it as their own to get your device banned somewhere you've never been, and two servers can line up their ban lists to prove two players are the same person. Per-server IDs make a stolen one useless anywhere but where it came from.",
		"Inside one server nothing changes: your ID there is the same every time you connect, so moderation and bans work exactly as they always did, and reconnecting over ws:// instead of wss:// (or with a differently-typed address) doesn't hand you a new one. The deliberate trade-off is that an HDID ban no longer follows your device to a DIFFERENT server. In practice that costs nothing, since no AO server software shares ban lists with any other server anyway.",
	}},
	{"How this differs from the standard AO2 client", []string{
		...
		"The classic client also sends that same value to EVERY server it joins, which is what makes it worth stealing: one copy identifies that PC everywhere. AsyncAO does neither. It hashes on your device first, mixes several roots so the fingerprint is harder to forge, and then derives a separate ID per server, so what one operator sees is inert on the next one.",
		"One trade-off to know: because the schemes differ, your AsyncAO HDID is a different value from the one AO2 would send for the same PC — they aren't interchangeable. You're still just as bannable on any given server; the difference is that your real account and hardware IDs stay on your computer, and no single value follows you around.",
	}},
	{"On the server side (IPID & HDID)", []string{
		...
		"As for the HDID: it's stored so a device can be banned. Only Athena and Nyathena hash it AGAIN on their side for extra privacy; the rest keep it as received. With AsyncAO that's a non-issue either way, because what you send is already a salted hash, and one that only means anything on that particular server — a server storing it 'as is' is holding an opaque token that's no use to anyone else, never your real IDs. (With the classic AO2 client, 'as is' would mean your raw account SID or hardware serial sitting in that database, and the same value would be sitting in every other server's database too.)",
	}},
	{"The web server in front can still see your IP", []string{
		"This is the catch worth knowing. The files your client downloads — character sprites, backgrounds, sounds — are handed out by web-server software, almost always a program called nginx or Caddy. Think of it as the front desk that actually receives your connection and passes out the files you ask for; the AO server itself sits behind it. (On an encrypted wss:// server, this front desk is also what handles the TLS.)",
		"Because that front desk is what your computer really connects to, it sees your real IP — and by default it writes that IP to a log EVERY time your client asks it for a file (and there are MANY such requests as a room loads), regardless of what the AO server software then does with it.",
