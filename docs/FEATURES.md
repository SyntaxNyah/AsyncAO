...
| Evidence | `LE`/`PE`/`DE`/`EE` + MS field | session, drawEvidencePanel | list/grid + inspector + editor; **icon-size slider**; **Choose** image picker (local mount `evidence/` in local/layered mode, the server's `evidence/` index in stream mode); centered/wrapped grid names; **Stop-editing confirm** on swap; **Keep-mine / Accept-theirs** compare on incoming `EE`; present arms the NEXT message (wire id +1, 0 = none, courtroom.cpp:2160); incoming presented evidence pops the image + IC log line; images stream as exact URLs (`evidence/<file.ext>` — extension ships in the name, zero probing) |
