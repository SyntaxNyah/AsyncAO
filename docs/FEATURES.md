...
| Evidence | `LE`/`PE`/`DE`/`EE` + MS field | session, drawEvidencePanel | list/grid + inspector + editor; **icon-size slider**; **Browse…** image picker follows the toggle — the **merged local asset sources** when local, the server's `evidence/` autoindex when streaming (local: seeds at `evidence/`, lists the first-source-wins union of every local mount, image-only, and clamped to the sources — a pick from `characters/` etc. is written `../characters/<name>/…`); centered/wrapped grid names; **Stop-editing confirm** on swap; **Keep-mine / Accept-theirs** compare on incoming `EE`; present arms the NEXT message (wire id +1, 0 = none, courtroom.cpp:2160); incoming presented evidence pops the image + IC log line; images read **local-first** by default (a **Stream from server** toggle in the panel flips to the server, mirroring the demo content report's source switch) as exact URLs (`evidence/<file.ext>` — extension ships in the name, zero probing) |
...
| Zoom emote (speedlines) | MS `emote_mod` 5/6 | courtroom `zoomEmote` + `ZoomSide`/`SpeedlinesCandidates`, viewport speedline layer | A ZOOM/preanim-zoom hides the desk + pair and plays the speaker's own radial burst (`<pos>_speedlines` then `<side>_speedlines`, first-resident-wins, bundled stock fallback); **every position resolves** — witness→prosecution, judge/jury/seance→defense (AO2's catch-all), never blank; the burst is a scheduled animation layer so it runs at full rate |
