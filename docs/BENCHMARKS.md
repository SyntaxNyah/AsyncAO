...
| UI Tab-chain reorder, 0 allocs | **0 allocs/op** — the AO2 chat chain (showname → IC → OOC → OOC name) is re-seated with a fixed stack scratch + one `copy`, on the Tab press only, never per frame | `TestOrderTabChainNoAlloc` |
